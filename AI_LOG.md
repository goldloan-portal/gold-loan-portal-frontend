...
- Prompt: Implement the first step of the loan intake flow (customer name, mobile number, gross/net weight, purity dropdown) via the worktree-based ticket workflow, validated client-side to mirror the backend's Zod rules; mid-session, redirected to rebuild the UI on shadcn/ui instead of hand-rolled form components.
